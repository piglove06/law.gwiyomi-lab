...
echo ============================================================
echo  Law Helper - git setup  (git jeojangso choogi seoljeong)
echo ============================================================
echo.
...
if not exist ".gitignore" (
  echo [STOP] .gitignore is missing. .env could be committed. Aborting.
  pause
  exit /b 1
)
...
if errorlevel 1 (
  echo.
  echo [STOP] Secret scan failed. Fix the items above, then run again.
  pause
  exit /b 1
)
...
echo.
echo ------------------------------------------------------------
echo  DONE. One manual step is left - sign in to GitHub once:
echo.
echo      git push -u origin main
echo.
echo  A browser window will open for GitHub login.
echo  After that, push happens automatically.
echo ------------------------------------------------------------
echo.
git status --short
pause
